...
export async function checkIfNewTransaction(orderModel, paypalTransactionId) {
  try {
    // Find all document where Order.paymentResult.id is the same as the id passed paypalTransactionId
    const orders = await orderModel.find({
      "paypalResult.id": paypalTransactionId,
    });

    // if there are not such orders, then it's a new transaction.
